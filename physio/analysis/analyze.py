...
def analyze(session, customCfgFile=None):
    ...
    config = cfg.load(session, customCfgFile)
    if not os.path.exists(config.get('session', 'outputprefix')):
        os.makedirs(config.get('session', 'outputprefix'))
    logging.root.addHandler(logging.FileHandler('%s/physio.log' % \
            config.get('session', 'outputprefix'), mode='w'))
    # config = cfg.Config()
    ...
    if len(epochs_audio) == 0:
        utils.error("No epochs found")

    # process pixel clock
    # [:,0] = audio, [:,1] = mworks
    matches, _ = clock.pixelclock.process_from_config(config)
    if len(matches) == 0:
        utils.error("No pixel clock matches found")
    ...
    for epoch_audio in epochs_audio:
        ...
        if not os.path.exists(config.get('session', 'output')):
            os.makedirs(config.get('session', 'output'))
        ...
        resultsFilename = config.get('session', 'output') + '/' + \
                session + '.h5'
        ...
        #offsetMatches[:,0] = (offsetMatches[:,0] - epoch_audio[0]) / \
        #        float(config.getint('audio','samprate'))
        #offsetMatches[:,0] = (offsetMatches[:,0] / \
        #        float(config.getint('audio','samprate'))) - epoch_audio[0]
        offsetMatches[:, 0] = (offsetMatches[:, 0] / \
                float(config.getint('audio', 'samprate')))
        h5.utils.write_array(resultsFilename, offsetMatches, \
                'TimeMatches', 'PC - MW Time Matches')

        # add session info
        logging.debug("adding sesison information")
        h5.utils.write_epoch_audio(resultsFilename, epoch_audio)
        h5.utils.write_dict(resultsFilename, sessionDict, \
                'SessionGData', 'Session data from GDocs')
        h5.utils.write_dict(resultsFilename, probeDict, \
                'ProbeGData', 'Probe data from GDocs')
        # results_file.add_pad_positions(pad_positions)
        h5.utils.write_git_commit_id(resultsFilename, \
                utils.get_git_commit_id())
