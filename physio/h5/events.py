...
def find_events_group(eventsFile):  # , regex = r'[a-z,A-Z]+[0-9]_[0-9]+'):
    ...
    for node in eventsFile:
        if type(node) != tables.group.Group:
            continue
        if not ('codec' in node):
            continue
        if not ('values' in node):
            continue
        if not ('events' in node):
            continue
        return node
...
def are_event_times_bad(h5file, event_name):
    ...
    if (event_name in ['path_origin_x', 'path_origin_y', 'path_origin_z',\
            ...
            and ('CNC_OFFSET' in h5file.root.Events._v_attrs._v_attrnames):
        return True
    elif (event_name in \
            ['cobra_timestamp', 'pupil_radius', 'gaze_h', 'gaze_v']) \
            and ('EYETRACKER_OFFSET' in \
            h5file.root.Events._v_attrs._v_attrnames):
        return True
    else:
        return False
...
def get_events(eventsFile, code, timeRange=None):
    ...
    with utils.H5Maker(eventsFile, 'r') as f:
        ...
        if type(code) != int:
            event_name = code
            codec = dict(g.codec.read())
            if not (code in codec.values()):
                utils.error('code[%s] not found in codec: %s' \
                        % (code, str(codec)))
        ...
        if are_event_times_bad(f, event_name):
            # because some conduit events were mistimed I can't just filter
            # by time. Instead, I need to get all the times, potentially fix
            # them and then throw out bad times
            evs = [(int(r['time']), g.values[r['index']]) for r in \
                    g.events.where('code == %i' % code)]
            if len(evs) != 0:
                ...
                if not (timeRange is None):
                    ...
                    evs = [ev for ev in evs if ((ev[0] > timeRange[0]) \
                            and (ev[0] <= timeRange[1]))]
        else:
            if timeRange is None:
                ...
                timeRange[0] = int(timeRange[0] * 1E6)
                timeRange[1] = int(timeRange[1] * 1E6)
                # PyTables version 2.2.1 does not support selection
                # on uint64 (the time type) so...
                assert np.iterable(timeRange), \
                        "timeRange[%s] must be iterable" % str(timeRange)
                assert len(timeRange) == 2, \
                        "timeRange length[%i] must be 2" % len(timeRange)
                assert type(timeRange[0]) == int, \
                        "timeRange[0] type[%s] must be int" % \
                        type(timeRange[0])
                evs = [(int(r['time']), g.values[r['index']]) for r \
                        in g.events.where('code == %i' % code) if \
                        int(r['time']) > timeRange[0] \
                        and int(r['time']) <= timeRange[1]]
...
def add_events_file(eventsFilename, dataFilename):
    ...
    dataFilename : string
         Filename or file (used with utils.H5Maker) containing other session
         data (spike events, etc...)
    """
    # eventsFile = tables.openFile(eventsFilename, 'r')
    # dataFile = tables.openFile(dataFilename, 'a')
    # make sure this is append and not write
    with utils.H5Maker(eventsFilename, 'r') as eventsFile:
        with utils.H5Maker(dataFilename, 'a') as dataFile:
            ...
            codecOut = dataFile.createTable(outGroup, 'codec', \
                    eventsGroup.codec.description)
            ...
            eventsOut = dataFile.createTable(outGroup, 'events', \
                    eventsGroup.events.description)
            ...
            valuesOut = dataFile.createVLArray(outGroup, 'values', \
